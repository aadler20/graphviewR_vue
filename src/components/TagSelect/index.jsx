...
export default {
  ...
  methods: {
    onChange (checked) {
      console.info('checked in onChange', checked)
      const key = Object.keys(this.items).filter(key => key === checked.value)
      // console.info('key in onChange', key)
      this.items[key] = checked.checked
      // console.info('this.items[key]', this.items[key])
    ...
    // render option
    renderTags (items) {
      // console.info('items in renderTags', items)
      const listeners = {
        change: (checked) => {
